  ...
  return (
    <SectionShell id="leistungen">
      <div className="text-center max-w-2xl mx-auto">
        <h2 className="text-2xl sm:text-4xl font-semibold">Leistungen</h2>
        <p className="mt-3 text-gray-600 dark:text-gray-300">Websites, KI-Lösungen und Automatisierungen – alles aus einer Hand.</p>
      </div>

      <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map(({ id, icon: Icon, title, bullets, priceHint }, idx) => (
          <motion.div
            key={id}
            whileHover={{ y: -4, rotate: -0.2 }}
            className="group relative rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-6 shadow-sm hover:shadow-md transition-all"
          >
            ...
            <div className="mt-4 text-xs text-gray-500 dark:text-gray-400">{priceHint}</div>
          </motion.div>
        ))}
      </div>
    </SectionShell>
